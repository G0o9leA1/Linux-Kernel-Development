...
int *num_array; // a pointer to an array of integers
...
static void cross_out(void *counter) {
	...
	while (1) {
		...
		curnt++;
		while (curnt < upper_bound-1 && num_array[curnt] == 0) {
			curnt++;
		}	
		spin_unlock(&lock);
...
static int my_init(void)
{
	...
	num_array = (int *) kmalloc(sizeof(int)*(upper_bound-1), GFP_KERNEL);
	if (!num_array) {
...
static void print_statistics(void) {
	...
	struct timespec diff1, diff2; //time difference of ts1 - ts0 and ts2 - ts1
