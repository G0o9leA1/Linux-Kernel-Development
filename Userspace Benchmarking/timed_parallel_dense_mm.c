...
int main( int argc, char* argv[] ){
	...
	if(argc < num_expected_args || argc > num_expected_args+1){
		printf("Usage: ./timed_parallel_dense_mm <size of matrices> <times of running>\n");
		exit(-1);
	}
	...
	for (t = 0; t < times; t++)
	{
		...
		long timestart = tpstart.tv_sec * BILLION + tpstart.tv_nsec;
	       	long timeend = tpend.tv_sec * BILLION + tpend.tv_nsec;
		long timespent = timeend - timestart;	
		sum += timespent;
		if (timespent < min) min = timespent;
		if (timespent > max) max = timespent;
	}
	long average = sum / times;
	printf("Minimum: %ld seconds, %ld nanoseconds\n", min/BILLION, min%BILLION);
	printf("Maximum: %ld seconds, %ld nanoseconds\n", max/BILLION, max%BILLION);
	printf("Average: %ld seconds, %ld nanoseconds\n", average/BILLION, average%BILLION);
	#ifdef VERIFY_PARALLEL
